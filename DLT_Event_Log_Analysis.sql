...
-- MAGIC %python
-- MAGIC dbutils.widgets.text('storage_location', '/pipelines/1b98baa4-85f3-4358-a102-1ffa94d249e3', 'Storage Location')
-- MAGIC dbutils.widgets.text('db', 'craig_lukasik_dlt_demo', 'DLT Database')
...
-- MAGIC   FROM pipeline_logs
-- MAGIC   where details:flow_progress.status in ('COMPLETED', 'RUNNING') and details:flow_progress.data_quality.expectations is not null
-- MAGIC   ORDER BY timestamp);
...
-- MAGIC %python
-- MAGIC 
-- MAGIC db = dbutils.widgets.get("db")
-- MAGIC 
